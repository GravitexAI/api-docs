...
// Configuration
const SOURCE_REPO = process.env.SOURCE_REPO || 'QuantumNous/gravitex-api';
...
const SPECIAL_THANKS_I18N = {
  zh: {
    ...
    sponsorsInfoDesc:
      '以下赞助商数据从爱发电平台自动获取。根据累计赞助金额，分为金牌、银牌和铜牌三个等级。如果您也想为项目提供资金支持，欢迎前往 [爱发电](https://afdian.com/a/gravitex-api) 平台进行捐赠。',
  ...
  ja: {
    ...
    sponsorsInfoDesc:
      '以下のスポンサーデータは、Afdian プラットフォームから自動的に取得されます。累計スポンサー金額に基づいて、ゴールド、シルバー、ブロンズの3つのレベルに分類されます。プロジェクトに財政的支援を提供したい場合は、[Afdian](https://afdian.com/a/gravitex-api) プラットフォームで寄付を歓迎します。',
