...
3. **新しいプロバイダーを追加**
   - 「モデルを追加」をクリック
   - Gravitex APIを選択
   ![Gravitex APIプロバイダーを追加](/assets/aionui/newapi_provider.png)

4. **API情報を設定**
   - APIアドレス：Gravitex APIサイトアドレスを入力（形式：`https://your-domain.com/v1`）
   - APIキー：Gravitex APIコンソールからコピーしたAPIキーを貼り付け

5. **モデルを追加**
   - ドロップダウンから追加するモデルを選択
   - モデル名はGravitex APIで設定したモデル名と一致する必要があります
   - 適切なリクエストプロトコルを選択

6. **使用開始**
   - チャットページに戻る
   - 設定したGravitex APIモデルを選択して会話を開始
