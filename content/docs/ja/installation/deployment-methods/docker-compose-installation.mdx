...
このドキュメントでは、Docker Compose を使用して Gravitex API をデプロイする詳細な手順を説明します。
...
1. Gravitex API デプロイ用のディレクトリを作成します。
...
- **指定サービスのログ**（例：`gravitex-api`、`mysql`、`redis`）
...
- **コンテナ名を使用してログを表示**（`container_name` が設定されている場合、例：設定内の `gravitex-api`）
