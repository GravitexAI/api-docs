...
## 在 LunaTranslator 接入 Gravitex API

LunaTranslator 支持接入本地部署的 Gravitex API 和第三方使用 Gravitex API 搭建的 Gravitex API 服务。
...
2. 在 **`Gravitex API`** -> `控制台` -> `令牌管理` 选项卡中选择要使用在 LunaTranslator 的令牌，点击聊天按钮旁的下拉选项，选择 `LunaTranslator`,将会跳转到 LunaTranslator 并自动配置 API 地址与 API Key
...
4. 点击 **model** 下拉框旁的刷新按钮，获取 Gravitex API 平台的模型列表，选择或输入模型名称，完成后点击确定保存
...
1. 在 **`Gravitex API`** -> `控制台` -> `令牌管理` 选项卡中获取 API Key
...
5. 点击 **model** 下拉框旁的刷新按钮，获取 Gravitex API 平台的模型列表，选择或输入模型名称，完成后点击确定保存

   ![设置 API3](/assets/luna_translator/setting_api3.png)

6. 点击 **Gravitex API** 旁边的开关按钮，启用接口即可开始使用
