...
以下是使用 Gravitex API 作为模型提供商的完整配置示例：
...
```json
{
  ...
  "agents": {
    "defaults": {
      "model": {
        "primary": "gravitexapi/gemini-3-flash-preview",
        "fallbacks": [
          "gravitexapi/kimi-k2.5"
        ]
      },
      "models": {
        "gravitexapi/gemini-3-flash-preview": {
...
| 配置项 | 说明 |
| --- | --- |
| `models.providers.gravitexapi.baseUrl` | Gravitex API 部署地址，需包含 `/v1` |
| `models.providers.gravitexapi.apiKey` | Gravitex API 令牌密钥 |
| `models.providers.gravitexapi.models` | 模型列表，可按需添加多个模型 |
