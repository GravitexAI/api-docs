...
This document provides methods and best practices for updating the Gravitex API system to ensure a smooth upgrade to the latest version.
...
2.  **Check update logs**: Review the update content for the latest version on [GitHub Releases](https://github.com/Calcium-Ion/gravitex-api/releases)
...
If you have deployed Gravitex API using a single Docker container, you can update by following these steps:

```shell
# 拉取最新镜像
docker pull calciumion/gravitex-api:latest

# 停止并移除旧容器
docker stop gravitex-api
docker rm gravitex-api

# 使用相同的参数重新运行容器
docker run --name gravitex-api -d --restart always \
  -p 3000:3000 \
  -e TZ=Asia/Shanghai \
  -v /your/data/path:/data \
  calciumion/gravitex-api:latest
```
...
2.  Find the Gravitex API application and click the **Upgrade** button
...
2.  Find the Gravitex API container, click **More** -> **Recreate**
...
If you have deployed Gravitex API by compiling from source code, the update steps are as follows:
...
```shell
# 拉取特定版本的镜像
docker pull calciumion/gravitex-api:v1.x.x
...
docker run --name gravitex-api -d --restart always \
  ...
  -e TZ=Asia/Shanghai \
  -v /your/data/path:/data \
  calciumion/gravitex-api:v1.x.x
```
