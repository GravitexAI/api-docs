...
This document details the Docker Compose configuration options for Gravitex API, applicable to various deployment scenarios.

## Basic Configuration Structure

The Docker Compose configuration file `docker-compose.yml` defines how the Gravitex API service and its dependent services (such as MySQL, Redis) are deployed.
...
services:
  gravitex-api:
    image: calciumion/gravitex-api:latest
    container_name: gravitex-api
    restart: always
    ...
    volumes:
      ...
      - ./logs:/app/logs
    environment:
      - SQL_DSN=postgresql://root:123456@postgres:5432/gravitex-api # ⚠️ IMPORTANT: Change the password in production!
      #      - SQL_DSN=root:123456@tcp(mysql:3306)/gravitex-api  # Point to the mysql service, uncomment if using MySQL
...
For testing purposes, you can use the following simplified version, which only includes the Gravitex API service itself:
...
### Gravitex API Service Configuration

| Parameter        | Description                                                                    |
| ---------------- | ------------------------------------------------------------------------------ |
| `image`          | Image name, usually `calciumion/gravitex-api:latest` to get the latest version      |
...
| `environment`    | Environment variable settings, used to configure Gravitex API behavior              |
...
Gravitex API supports various environment variable configurations. Here are some commonly used ones:

| Environment Variable | Description                                         | Example                               |
| -------------------- | --------------------------------------------------- | ------------------------------------- |
| `SQL_DSN`            | Database connection string                          | `root:123456@tcp(mysql:3306)/gravitex-api` |
...
services:
  gravitex-api-master:
    image: calciumion/gravitex-api:latest
    container_name: gravitex-api-master
    restart: always
    ports:
      - '3000:3000'
    environment:
      - SQL_DSN=root:123456@tcp(mysql:3306)/gravitex-api
      - REDIS_CONN_STRING=redis://redis
...
services:
  gravitex-api-slave:
    image: calciumion/gravitex-api:latest
    container_name: gravitex-api-slave
    restart: always
    ports:
