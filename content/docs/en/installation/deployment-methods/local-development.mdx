...
This document provides detailed steps for setting up and developing the Gravitex API project in a local environment, suitable for developers who wish to contribute to the project or perform custom development.
...
First, clone the Gravitex API repository from GitHub to your local machine:

```bash
git clone https://github.com/Calcium-Ion/gravitex-api.git
cd gravitex-api
```
...
Gravitex API supports configuring environment variables via a `.env` file. Create a `.env` file (you can copy from `.env.example`):
...
```bash
PORT=3000
SQL_DSN=root:password@tcp(localhost:3306)/gravitex-api   # If using MySQL, uncomment and modify
...
The frontend code for Gravitex API is located in the `web` directory and is developed using React and the [semi design component library](https://semi.design/zh-CN).
...
The directory structure of the Gravitex API project:

```
gravitex-api/                                 # Project root directory
...
1.  Check [GitHub Issues](https://github.com/Calcium-Ion/gravitex-api/issues)
