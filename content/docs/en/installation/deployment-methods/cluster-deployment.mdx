...
This document provides detailed configuration steps and best practices for Gravitex API cluster deployment, helping you build a highly available, load-balanced distributed system.
...
The Gravitex API cluster adopts a master-slave architecture design:
...
services:
  gravitex-api-master:
    ...
    ports:
      - '3000:3000'
    environment:
      - SQL_DSN=root:password@tcp(your-db-host:3306)/gravitex-api
      ...
      # - FRONTEND_BASE_URL=https://<your-gravitexapi-domain> # 前端基础 URL，用于邮件通知等功能
    volumes:
      - ./data:/data
      - ./logs:/app/logs
```
...
services:
  gravitex-api-slave:
    ...
    environment:
      - SQL_DSN=root:password@tcp(your-db-host:3306)/gravitex-api # 与主节点相同
      ...
      - TZ=Asia/Shanghai
      # 以下是可选配置
      # - FRONTEND_BASE_URL=https://<your-gravitexapi-domain> # 需与主节点相同
    volumes:
      - ./data:/data
      - ./logs:/app/logs
```
...
server {
    listen 80;
    server_name <your-gravitexapi-domain>;
