...
This document provides detailed steps for deploying Gravitex API using Docker Compose.
...
git clone https://github.com/QuantumNous/gravitex-api.git
...
1. Create a directory for Gravitex API deployment:
...
- **Logs for a specific service** (Examples: `gravitex-api`, `mysql`, `redis`)

```bash
docker compose logs -f gravitex-api
...
```bash
docker compose logs --tail=100 gravitex-api
```

- **View logs from a recent period**

```bash
docker compose logs --since=10m gravitex-api
```

- **Show timestamps**

```bash
docker compose logs -f -t gravitex-api
```
...
docker compose up gravitex-api
```
...
- **View logs using container name** (when `container_name` is set, e.g., `gravitex-api` in the configuration)

```bash
docker logs -f gravitex-api
```
