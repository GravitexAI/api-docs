...
description: OpenClaw Tutorial — Install OpenClaw, integrate with Gravitex API, and quickly set up a self-hosted AI assistant. An open-source project supporting multi-channel integration like Telegram, Discord, WhatsApp.
---
...
OpenClaw is completely open source. You can browse the source code, submit issues, or contribute at [OpenClaw's GitHub repository](https://github.com/openclaw/openclaw). This tutorial covers the complete steps for installation, configuration, and integrating OpenClaw with Gravitex API.
...
Before integrating with Gravitex API, it's recommended to first get the Gateway and Control UI running according to OpenClaw's currently recommended official process. This makes it easier to distinguish whether OpenClaw itself hasn't started or if the model provider configuration is incorrect when troubleshooting later.
...
This wizard completes basic authentication, Gateway setup, and optional channel initialization. The goal here is to get OpenClaw running first, then switch the default model to Gravitex API later.
...
OpenClaw supports integrating custom or OpenAI-compatible model gateways via `models.providers`. For Gravitex API, the most common approach is to add it as a custom provider to the configuration, then point the default model to `newapi/MODEL_ID`.
...
First, provide your Gravitex API key in the current shell, service environment, or a `.env` file readable by OpenClaw:
...
This is not a complete configuration to be copied verbatim, but rather the most critical part for integrating Gravitex API. As long as the `provider`, model ID, and default model references are correctly matched, OpenClaw will be able to call the model resources you expose via Gravitex API.
...
| `models.providers.newapi.baseUrl` | Your Gravitex API address, usually needs to include `/v1`. |
| `models.providers.newapi.apiKey` | Gravitex API key, recommended to inject via `${NEWAPI_API_KEY}`. |
| `models.providers.newapi.api` | For OpenAI-compatible gateways like Gravitex API, use `openai-completions`. |
| `models.providers.newapi.models` | The model IDs listed here must match the actual model names exposed by your Gravitex API. |
