...
Here is a complete configuration example using Gravitex API as the model provider:
...
| --- | --- |
| `models.providers.newapi.baseUrl` | Gravitex API deployment address, must include `/v1` |
| `models.providers.newapi.apiKey` | Gravitex API token key |
