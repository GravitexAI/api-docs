...
description: LunaTranslator setup guide — open-source GalGame and visual novel translator with HOOK, OCR, and AI-powered translation. Integrate with Gravitex API for enhanced accuracy.
---

<Callout type="info" title="Chat Settings Options">
  In the Gravitex API console's System Settings -> Chat Settings, you can add the
...
## Integrating Gravitex API into LunaTranslator

LunaTranslator supports integrating with locally deployed Gravitex API and third-party Gravitex API services built using Gravitex API.
...
2.  In the **`Gravitex API`** -> `Console` -> `Token Management` tab, select the token to be used in LunaTranslator, click the dropdown option next to the chat button, select `LunaTranslator`, and it will jump to LunaTranslator and automatically configure the API address and API Key.
...
4.  Click the refresh button next to the **model** dropdown box to get the Gravitex API platform's model list, select or enter the model name, then click OK to save.
...
1.  In the **`Gravitex API`** -> `Console` -> `Token Management` tab, obtain the API Key.
...
5.  Click the refresh button next to the **model** dropdown box to get the Gravitex API platform's model list, select or enter the model name, then click OK to save.

    ![设置 API3](/assets/luna_translator/setting_api3.png)

6.  Click the toggle button next to **Gravitex API** to enable the interface and start using it.
