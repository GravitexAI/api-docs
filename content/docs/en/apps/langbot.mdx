...
description: LangBot integration guide — build AI-powered chatbots for Feishu, DingTalk, Telegram, Discord, and more. Supports Knowledge Base, Agent, and MCP with Gravitex API.
---
...
<Callout type="info">
  ...
  Agent, MCP, and is perfectly compatible with Gravitex API.
</Callout>
...
## Integrating Gravitex API

LangBot supports integrating with locally deployed Gravitex API and third-party Gravitex API services built using Gravitex API.

### Usage

1.  Obtain an API key from Gravitex API
    ![获取 API key](/assets/langbot/get_api_key.png)

        If Gravitex API is deployed locally, please configure the API address yourself (refer to [Container Network Connection](https://docs.langbot.app/en/workshop/network-details.html)). If using a third-party Gravitex API service, you can copy the address from the page. Note that `/v1` needs to be appended to the address.

2.  Add a model in LangBot, select Gravitex API as the provider, and fill in the corresponding API key and API address
    ![添加 Gravitex API 模型](/assets/langbot/add_gravitexapi_model.png)
...
LangBot supports using Gravitex API's embedding models and utilizing them as vector models for the knowledge base.

1. Add an embedding model in LangBot, select Gravitex API as the provider
