...
description: AionUi Tutorial — A free open-source desktop office agent supporting various AI agents like Gemini CLI, Claude Code, Codex, and more. Integrates with Gravitex API to enable multi-model workflows.
---
...
Provider Type: Types supported by GravitexApi
API Key: Obtained from GravitexApi
API Address: GravitexApi site address (e.g., `https://<your-gravitexapi-domain>/v1`)

Please use your self-deployed Gravitex API, or ensure that the service provider offers Gravitex API services with legitimate upstream authorization and compliance obligations. Do not connect API addresses or keys from unknown sources to a production environment.

### Configuration Steps

1.  **Copy API key in GravitexApi**
...
3.  **Add New Provider**
    - Click "Add Model"
    - Select GravitexApi
    ![Add GravitexApi Provider](/assets/aionui/gravitexapi_provider.png)

4.  **Configure API Information**
    - API Address: Enter your GravitexApi site address (format: `https://<your-gravitexapi-domain>/v1`)
    - API Key: Paste the API Key copied from the GravitexApi console

5.  **Add Model**
    - Select the model to add from the dropdown
    - The model name should match the model name configured in GravitexApi
    - Select the appropriate request protocol

6.  **Start Using**
    - Return to the chat page
    - Select the configured GravitexApi model to start a conversation
