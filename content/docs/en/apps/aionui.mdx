...
description: AionUi setup guide — free, open-source desktop office agent supporting Gemini CLI, Claude Code, Codex, and more. Connect to Gravitex API for multi-model AI workflows.
---
...
## Gravitex API Configuration Steps
...
3. **Add New Provider**
   - Click "Add Model"
   - Select Gravitex API
   ![Add Gravitex API Provider](/assets/aionui/newapi_provider.png)

4. **Configure API Information**
   - API Address: Fill in your Gravitex API site address (format: `https://your-domain.com/v1`)
   - API Key: Paste the API Key copied from Gravitex API console

5. **Add Models**
   - Select the model to add from the dropdown
   - Model name should match the model name configured in Gravitex API
   - Select the appropriate request protocol

6. **Start Using**
   - Return to the chat page
   - Select the configured Gravitex API model to start a conversation
